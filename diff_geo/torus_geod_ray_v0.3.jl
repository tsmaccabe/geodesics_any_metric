...
dt = 0.01
q_ode = SecondOrderODEProblem(ddq, w0, q0, (0, max_dist), (a, b), saveat=dt)
...
plot_positive!(p) = scatter!(p, xi_n, yi_n, zi_n, legend=false, camera=cam_angle, xlims=[-(a+b), (a+b)], ylims=[-(a+b), (a+b)], zlims=[-(a+b), (a+b)], aspect_ratio=:equal, markersize=0.8, markerstrokewidth=0., markercolor=:blue)

plot_negative!(p) = scatter!(p, xi_p, yi_p, zi_p, legend=false, camera=cam_angle, xlims=[-(a+b), (a+b)], ylims=[-(a+b), (a+b)], zlims=[-(a+b), (a+b)], aspect_ratio=:equal, markersize=0.8, markerstrokewidth=0., markercolor=:orange)
